...
var civilians = sum( operationStarships.map(
    starship => operation[starship.name] * parseInt(starship.passengers, 10)
) );
console.log('civilians = ' + civilians);

// How many crew members would be required for the same operation?
var crew = sum( operationStarships.map(
    starship => operation[starship.name] * parseInt(starship.crew, 10)
) );
console.log('crew = ' + crew);

// Total cost of the operation?
// 1 crew = 10 credit
var cost = sum( operationStarships.map(
    starship => operation[starship.name] * (parseInt(starship.cost_in_credits, 10) + parseInt(starship.crew, 10) * 10)
) );
